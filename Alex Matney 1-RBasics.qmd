...
```{r}
paste("wow","look at me code lol")
```

(@) Can you explain what happened in 2-3 sentences? 

> what is happening is that we are making messsages pop up. as well as we are having math and numbers be done, but thats that is just the tools that we are using. these are tools that are being used to make what we need possable, as we are putting in what we need and having a function do what we need by imputing the commands in a way that they will run. 
...
> Each one os a diffrent meaning/code if two numbers are less than equal to or all of the diffrent codes it to use logic. 5 is not equal to 10, but 5 is equal to 5. It uses logic to compare the numerical value of the number. 
...
grade1 <- 70
grade4 <- 50
grade1_times_grade4 <- grade1 * grade4
```
...
> The vector my.numbers was multiplied by 2 and produced an output for each value in the vector.
...
>We only multiplied 6 by 2 because 6 is the third number in the vector.

(@) You try, create a list of five items and store it in a descriptive variable. 
```{r}
my_personal_vector <- c(4,6,10,14,16)
my_personal_vector
```
...
In real life, you typically you wouldn't want to store this code in your script file. You probably don't need to pull up the documentation for the function every time you run that piece of code. But for the purposes of this worksheet we're adding it to our `.qmd` document. 
...
>Each number 1-5 which is x is being multiplied by 2 and then added to y which is 1, and that is where the z value comes to be. 


## Loading Dataframes from Packages

Lets also install the data package for our class. This package contains a variety of historical datasets that you can use to complete your assignments this semester. However, this package is not on CRAN. That's okay -- we can still install it from GitHub. To do that we need a package that knows how to install *other* packages from GitHub. We'll use `pak`.

First, install `pak` by running this in your **console** (not in this document):

```
install.packages("pak")
```

Our class's package is called `DigitalMethodsData` and it's hosted on [GitHub](https://github.com/lukeave/DigitalMethodsData). Now that you have `pak`, install it by running this in your console:

```
pak::pak("lukeave/DigitalMethodsData")
```

The `user/repository` format is how you point at a package on GitHub.

> **If you get an error that says `HTTP error 401. Bad credentials`:** R has found an expired GitHub token saved on your computer. Run `Sys.unsetenv("GITHUB_PAT")` in your console and try the install again. This trips up a lot of people and has nothing to do with your code.
...
>This dataset contains information transcribed from Bob Damron's Address Books from 1965 through 1985. Last updated December 2022. this is what the dataset is.
...
>I Read the documentation, and also used the command that let me ask what a function does. I was then told about head and tail. 
...
>gayguides is the dataset, $ and title is the data segment, 500 is the 500th title. 
...
>It seems to me that there is data here that tells about the judges and a bit about them. Their positions, and what they did and I saw some things about military service. Seems a little poorly formated.
